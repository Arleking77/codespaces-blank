...
export default function VentasScreen({ clientes, precios, ciudades, cargarDatos }: VentasScreenProps) {
  ...
  const [cantidadVenta, setCantidadVenta] = useState<string>('1'); // Guardado como string para permitir edición manual fluida
  ...
  const handleConfirmarVenta = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === '23503') {
          throw new Error("El cliente o producto seleccionado no está registrado en el servidor de Supabase.");
        }
        throw error;
      }
